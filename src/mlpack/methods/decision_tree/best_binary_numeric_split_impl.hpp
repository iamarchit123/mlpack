...
//if labels has size_t type use for classification
...
//If predictors if of type arma::rowvec use regression
template<typename FitnessFunction>
template<bool UseWeights, typename VecType, typename WeightVecType>
double BestBinaryNumericSplit<FitnessFunction>::SplitIfBetter(
    const double bestGain,
    ...
    AuxiliarySplitInfo<typename VecType::elem_type>& /* aux */)
{
  ...
  // Next, sort the data.
  arma::uvec sortedIndices = arma::sort_index(data);
  arma::rowvec sortedPredictors(predictors.n_elem);
  arma::rowvec sortedWeights;
  for (size_t i = 0; i < predictors.n_elem; ++i)
    sortedPredictors[i] = predictors[sortedIndices[i]];
  ...
  if (UseWeights)
  {
    sortedWeights.set_size(sortedPredictors.n_elem);
    // The weights must keep the same order as the labels.
    for (size_t i = 0; i < sortedPredictors.n_elem; ++i)
      sortedWeights[i] = weights[sortedIndices[i]];
  }
  ...
  for (size_t index = minimum; index < data.n_elem - minimum; ++index)
  {
    ...
    // Calculate the gain for the left and right child.  Only use weights if
    // needed.
    const double leftGain = FitnessFunction::template Evaluate<UseWeights>
                      (sortedPredictors, sortedWeights, 0, index-1);
    const double rightGain = FitnessFunction::template Evaluate<UseWeights>
                      (sortedPredictors, sortedWeights, index, data.n_elem-1);
    ...
    else
    {
      // Calculate the gain at this split point.
      gain = double(index) * leftGain +
          double(predictors.n_elem - index) * rightGain;
    }
  ...
  if (UseWeights)
    ...
    bestFoundGain /= sortedPredictors.n_elem;
